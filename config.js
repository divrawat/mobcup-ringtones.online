// export const DOMAIN = 'http://localhost:3000';
// export const BACKEND_DOMAIN = 'http://localhost:8000';

export const DOMAIN = 'https://mobcup-ringtones.online';
export const BACKEND_DOMAIN = 'https://ringtones-backend.vercel.app'



export const DOMAIN_NAME = 'mobcup-ringtones.online';
export const R2_SUBDOMAIN = "https://songs.mobcup-ringtones.online";
export const DISQUS_SHORTNAME = "my-cms-7";


export const APP_NAME = 'Mobcup';
export const NavbarName = "Mobcup";

export const APP_LOGO = `${DOMAIN}/logo.png`;
export const NOT_FOUND_IMAGE = `${R2_SUBDOMAIN}/public/404.webp`;
export const FAVICON = `${R2_SUBDOMAIN}/public/favicon.png`;

export const navLinks = [
    // { text: 'Rap', href: `${DOMAIN}/category/rap?page=1` },
    // { text: 'Hindi', href: `${DOMAIN}/category/hindi?page=1` },
    // { text: 'Punjabi', href: `${DOMAIN}/category/punjabi?page=1` },
    // { text: 'Haryanvi', href: `${DOMAIN}/category/haryanvi?page=1` },
    // { text: 'Bhajan', href: `${DOMAIN}/category/bhajan?page=1` },
    // { text: 'Bhakti', href: `${DOMAIN}/category/bhakti?page=1` },
    // { text: 'DJ', href: `${DOMAIN}/category/dj?page=1` },
    { text: 'Lofi', href: `${DOMAIN}/category/lofi?page=1` },
    { text: 'Mashup', href: `${DOMAIN}/category/mashup?page=1` },
    // { text: 'Remix', href: `${DOMAIN}/category/remix?page=1` },
];

export const FooterLinks = [
    { text: 'About', href: `${DOMAIN}/about` },
    { text: 'Contact', href: `${DOMAIN}/contact` },
    { text: 'Disclaimer', href: `${DOMAIN}/disclaimer` },
    { text: 'Privacy Policy', href: `${DOMAIN}/privacy-policy` },
    { text: 'Terms & Conditions', href: `${DOMAIN}/terms-and-conditions` }
];